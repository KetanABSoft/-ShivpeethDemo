...
import 'package:messeges_with_firebase/shivpeeth/shivpeeth_pages/shivpeeth_student/shivpeeth_stview_monthly.dart';
import 'dart:convert';
import 'dart:async';
...
import '../shivpeeth_home/shivpeeth_dailyattendence.dart';
import '../shivpeeth_theme/shivpeeth_themecontroller.dart';


class Wir_student_att_dash extends StatefulWidget {
  final String?classid;
  final String?sectionid;
  final String?type;
  final String?date_pass;

  const Wir_student_att_dash({
  ...
  @override
  State<Wir_student_att_dash> createState() => Wir_student_att_dash_state();
}
...
class Wir_student_att_dash_state extends State<Wir_student_att_dash> {
  dynamic size;
  double height = 0.00;
  double width = 0.00;
...
void initState() {
  super.initState();
   ...
   void getshadpreference()async {
     ...
     print("classid"+widget.classid.toString());
...
void openUrl() async {
  ...
  DateTime date;
    if (widget.date_pass != null) {
     ...
     String ?datepass= date != null ? date.toUtc().toIso8601String() : null;
     String appUrlNew = AppString.loginurl.replaceAll("https", "http") +
    'exporttoecel_stud_parentattenMonthly.php?Monthly=$fetchdata&id=$id&role=$role&branch_id=$branch_id&classid=$class_id&sectionid=$section_id&status=$status&date=$datepass';
                      ...
                      children: [
                        ...
                        Container(
                          child: fetchdata == "Daily"
                              ...
                              : fetchdata == "Monthly"
                                  ? studview_monthlyattendence(classid:widget.classid,sectionid:widget.sectionid,type:widget.type,date_pass:widget.date_pass)
                                  : Container(),
                        ),
                      ],
                    ),
