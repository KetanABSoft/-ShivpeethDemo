...
import '../shivpeeth_adapter/studentfee_reports.dart';
...
class studentfeereportteshow extends StatefulWidget {
  final String studentid;
  const studentfeereportteshow({
    required this.studentid,
    super.key});

  @override
  State<studentfeereportteshow> createState() => studentfeereportteshowState();
}

class studentfeereportteshowState extends State<studentfeereportteshow> {
   double height = 0.00;
  double width = 0.00;
  ...
  Future<void> studentfeereport() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
     roleid = preferences.getString('role');
       ...
       print(userid);
       print(session_id);
 String apiUrl = AppString.constanturl + 'Fetch_stfeesdata_teachershow';
    final response = await http.post(
      Uri.parse(apiUrl),
      body: {
        "id": widget.studentid != null ? widget.studentid : 'userid',
